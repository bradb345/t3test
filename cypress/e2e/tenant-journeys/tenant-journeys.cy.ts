...
describe("Tenant Journeys", () => {
  ...
  // ─── Journey 3.5: Document Verification ──────────────────────────

  describe("Journey 3.5: Document Verification", () => {
    it("landlord approves a document", () => {
      loginAsLandlord();
      cy.visit("/my-properties?tab=documents");

      // Wait for the documents tab to load
      cy.contains("Documents", { timeout: 15000 }).should("be.visible");

      // Verify tenant documents are visible with pending review status
      cy.contains("pending review", { timeout: 10000 }).should("be.visible");

      // Find a pending document and approve it
      ...
      cy.contains("Document approved successfully", { timeout: 10000 }).should(
        "be.visible"
      );
    ...
    it("landlord rejects a document", () => {
      loginAsLandlord();
      cy.visit("/my-properties?tab=documents");
      ...
      // Find a pending document and reject it
      cy.contains("pending review", { timeout: 10000 })
        .first()
        .closest("[class*='py-3']")
        .within(() => {
          ...
          cy.get("textarea").type(
            "Image is too blurry, please re-upload a clearer photo"
          );
      ...
      // Verify success toast
      cy.contains("Document rejected successfully", { timeout: 10000 }).should(
        "be.visible"
      );
  ...
  // ─── Journey 4.5: Tenant Document Re-upload ─────────────────────
  // (Runs after lease signing so the tenant can access the dashboard)

  describe("Journey 4.5: Tenant Document Re-upload", () => {
    it("tenant sees verification status and re-uploads rejected document", () => {
      loginAsTenant();
      cy.visit("/dashboard");

      // Navigate to Documents tab
      cy.contains("Documents", { timeout: 15000 }).click();

      // Verify approved document shows "Approved" badge
      cy.contains("Approved", { timeout: 10000 }).should("be.visible");

      // Verify rejected document shows "Rejected" badge with notes
      cy.contains("Rejected").should("be.visible");
      cy.contains("Image is too blurry").should("be.visible");

      // Click Re-upload on rejected document
      cy.contains("Re-upload").click();

      // Upload modal should open with pre-selected document type
      cy.contains("Upload Document", { timeout: 10000 }).should("be.visible");

      // Scope interactions to the dialog to avoid matching background elements
      cy.get('[role="dialog"]').within(() => {
        // Upload new file
        ...
        // Wait for file to be selected (file name appears in modal)
        cy.contains("property-image.jpg", { timeout: 5000 }).should(
          "be.visible"
        );

        // Submit (force: true needed — Radix dialog scroll-lock sets
        // pointer-events: none on body)
        cy.contains("button", "Upload").click({ force: true });
      });

      // Verify success
      cy.contains("Document uploaded successfully", { timeout: 30000 }).should(
        "be.visible"
      );

      // Verify new document appears with Pending Review status
      cy.contains("Pending Review").should("be.visible");
    });

    it("landlord approves the re-uploaded document", () => {
      loginAsLandlord();
      cy.visit("/my-properties?tab=documents");
      ...
      // Find the re-uploaded document with pending review status
      cy.contains("pending review", { timeout: 10000 })
        .first()
        .closest("[class*='py-3']")
        .within(() => {
